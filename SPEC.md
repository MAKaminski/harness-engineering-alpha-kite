...
Reference: [https://developers.openai.com/codex/app-server/](https://developers.openai.com/codex/app-server/)
...
function reconcile_running_issues(state):
  ...
  for issue in refreshed:
    ...
    if issue.state in terminal_states:
      ...
      state = terminate_running_issue(state, [issue.id](http://issue.id), cleanup_workspace=true)

    else if issue.state in active_states:

      state.running[[issue.id](http://issue.id)].issue = issue

    else:

      state = terminate_running_issue(state, [issue.id](http://issue.id), cleanup_workspace=false)
...
function dispatch_issue(issue, state, attempt):
  ...
  if worker spawn failed:
    ...
    return schedule_retry(state, [issue.id](http://issue.id), next_attempt(attempt), {
  ...
  state.running[[issue.id](http://issue.id)] = {
  ...
  state.claimed.add([issue.id](http://issue.id))

  state.retry_attempts.remove([issue.id](http://issue.id))
...
function run_agent_attempt(issue, attempt, orchestrator_channel):
  ...
  while true:
    ...
    turn_result = app_[server.run](http://server.run)_turn(
      ...
      on_message=(msg) -> send(orchestrator_channel, {codex_update, [issue.id](http://issue.id), msg})
    ...
    refreshed_issue = tracker.fetch_issue_states_by_ids([[issue.id](http://issue.id)])
